...
Transforming the time from POSIXct to date-time format using lubridate package.
...
df %>%
  ...
  summarise(pickup_tip = mean(tip_amount), num_trips = n()) %>%
  arrange(desc(pickup_tip))


df %>%
  group_by(do_location_id) %>%
  summarise(dropoff_tip = mean(tip_amount), num_trips = n()) %>%
  arrange(desc(dropoff_tip))
